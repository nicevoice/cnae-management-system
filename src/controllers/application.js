var config = require('../config')
 ...
 exports.loadMainContent = function(req, res){
    ...
    //从app_mem中查找参与的应用
    find(app_mem, {email:req.session.email.toString(),
    role:{$ne:0}, active:{$ne:2}}, {appDomain:1, appName:1, active:1}, {sort:[['joinTime', 1]]}, function(err, data){
    	if(err){
    		log.error(err.toString());
 ...
 if(newAppDes.length>100){
  ...
  checkRepetition.assign("checkDomain", "checkNumbers", function(goodDomain, checkNumbers){
 		if(goodDomain===1)
            return res.render("newApp", {
                layout:"layoutMain",
                warn:{domainWarn:"此域名已被占用"},
                email:req.session.email,
                nickName:req.session.nickName});
 		if(checkNumbers===1)
            return res.render("newApp", {
                layout:"layoutMain",
                warn:{domainWarn:"创建的应用数量达到上限"},
                email:req.session.email,
                nickName:req.session.nickName});
