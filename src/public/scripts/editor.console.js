...
CLI.loader = {
	...
	init: function() {
		...
		// 初始化console样式
		CLI.cache.location = cookieHandler.get(NAEIDE_config.COOKIE.console_location) || "BOTTOM";
		$("#console-location option[value='" + CLI.cache.location + "']").attr("selected", "selected");
		$("#console-display").removeAttr("disabled");
		_display = cookieHandler.get(NAEIDE_config.COOKIE.console_display);
	...
	locate: function(loc) {
		...
		if (loc === "BOTTOM") {
			...
			cookieHandler.set(NAEIDE_config.COOKIE.console_location, CLI.cache.location);
	...
	display: function(show) {
		if (!show) { // 隐藏
			...
			cookieHandler.set(NAEIDE_config.COOKIE.console_display, '0');
		} else { // 显示
			...
			cookieHandler.set(NAEIDE_config.COOKIE.console_display, '1');
		}
	},
