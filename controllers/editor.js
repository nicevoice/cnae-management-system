...
exports.index = function(req, res) {
	...
	res.render("editor", {domain: domain, layout: false});
}
...
exports.listfile = function(req, res) {
	var resMsg = {status: LANG_ERROR, content: null};
	var dirPath = req.body.dirPath || "";
	var domain = req.params.id;
	dirPath = path.join(ROOT_PATH, domain, dirPath);
	fs.readdir(dirPath, function(err, items) {
		if(err) return res.json(setError("failed to read dir"), 200);
		// 格式化输出的内容
		...
		for(var k in items) {
			// 过滤隐藏文件和临时文件
			if(regExp.test(items[k])) continue;
			var _f = {};
			_f.name = items[k]; // 文件名
			// 文件类型
			try {
				var _stat = fs.statSync(path.join(dirPath, _f.name));
				if(_stat.isFile()) _f.type = 'f';
				else if(_stat.isDirectory()) _f.type = 'd';
				else continue; // 只显示文件和文件夹
...
exports.readfile = function(req, res) {
	...
	fs.readFile(filePath, "utf8", function(err, data) {
		if(err) return res.json(setError("failed to read file"), 200);
		resMsg.status = LANG_SUCCEED;
...
exports.writefile = function(req, res) {
	...
	fs.writeFile(filePath, content, function(err) {
		if(err) return res.json(setError("failed to write file"), 200);
		// 获取文件最新的mtime
		fs.stat(filePath, function(err, stats) {
			if(err) return res.json(setError("failed to get file stats"), 200);
...
exports.renamefile = function(req, res) {
	...
	fs.rename(oriPath, newPath, function(err) {
		if(err) return res.json(setError("failed to rename file"), 200);
		// 获取文件最新的stat
...
exports.delfile = function(req, res) {
	...
	fs.unlink(filePath, function(err) {
		if(err) return res.json(setError("failed to remove file"), 200);
		resMsg.status = LANG_SUCCEED;
		return res.json(resMsg, 200);
	});
}
...
exports.mkdir = function(req, res) {
	...
	fs.mkdir(dirPath, 0755, function(err) {
		if(err) return res.json(setError("failed to make dir"), 200);
		resMsg.status = LANG_SUCCEED;
		return res.json(resMsg, 200);
	});
}
...
exports.deldir = function(req, res) {
	...
	rimraf(dirPath, function(err) {
		if(err) return res.json(setError("failed to remove dir"), 200);
		resMsg.status = LANG_SUCCEED;
		return res.json(resMsg, 200);
	});
}
