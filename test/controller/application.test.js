var testCase = require('nodeunit').testCase,
    ...
    application = require('../../src/controllers/application'),
...
var wrongCase = [{domain:"1unit", name:"1unit", except:{page:"error",data:{message:"子域名格式错误"}}},
                 {domain:"unit", name:"", except:{page:"error",data:{message:"必须有应用名称"}}},
                 {domain:"unit1", name:"unit1", except:{page:"error",data:{message:"该域名已经被占用"}}},
                 {domain:"unit11", name:"unit11", except:{page:"error",data:{message:"创建的应用数目已经达到上限"}}}];
var rightCase = [{domain:"unit1", name:"unit1", except:"/application"}];
...
module.exports = testCase({
  ...
  test_create_db : function(test){
    ...
    count(collections.app_member, {email:"unitTest@gmail.com", appDomain:"1unit"}, function(err, count){
      test.deepEqual(count, 0, "test wrong domain db member error");
      done();
    })
    count(collections.app_basic, {appDomain:"1unit"}, function(err, count){
      test.deepEqual(count, 0, "test wrong domain db basic error");
      done();
    })
    count(collections.app_member, {email:"unitTest@gmail.com", appDomain:"unit1"}, function(err, count){
      test.deepEqual(count, 1, "test right domain db member error");
      done();
    });
    count(collections.app_basic, {appDomain:"unit1"}, function(err, count){
  ...
  test_delete_app_wrong_user : function(test){
    ...
    testEvent.once('testNow', function(data){
      test.deepEqual(data, {status:"error", msg:"该应用不存在或没有权限删除此应用"}, "test delete other's app error");
      test.done();
    });
  ...
  test_delet_app_wrong_domain : function(test){
    ...
    testEvent.once('testNow', function(data){
      test.deepEqual(data, {status:"error", msg:"该应用不存在或没有权限删除此应用"}, "test delete not exist app error");
      test.done();
    });
    application.deleteApp(req, res);
  },
  test_delet_app_success:function(test){
    ...
    req.body.domain = "unit1";
    testEvent.once('testNow', function(data){
      test.deepEqual(data, {status:"ok"}, "test delete other's app error");
      test.done();
    });
    application.deleteApp(req, res);   
  },
