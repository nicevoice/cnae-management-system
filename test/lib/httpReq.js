var http = require('http'), url = require('url'), httpReq = require('../../src/lib/httpReq').httpReq, testCase = require('nodeunit').testCase;
...
module.exports = testCase({
  ...
  test_good_path_with_query : function(test) {
      httpReq(optionsQuery, function(data) {
        test.deepEqual(data, {status:"ok", query:"unitTest"}, "http req with query error");
        test.done();
    });
  },
  ...
  test_good_path_without_query : function(test){
    httpReq(optionsNoQuery, function(data){
        test.deepEqual(data, {status:"error", msg:"no query test"}, "http req without query error");
        test.done();      
    })
  },
